  ...
  * */
  parseUrlData(time) {
    ...
    if (!_.isEmpty(time)) {
      ...
      params = {year: time.year, month: time.month, day: time.day, rtp_token: '1'}
    } else {
      let day = null
      let hour = Moment().hour()
      if (hour >= 10) {
      ...
      let date = Moment(day)
      this.$store.getters.reportDate = date
  ...
  sum(arr, timeType, obj) {
    let result = 0
    _(arr).forEach((item) => {
      if (item && item[timeType] && item[timeType][obj]) {
        result += item[timeType][obj]
      }
    })
    return result
  },
